...
import { createCommand } from './operator-api';
...
export const CommandControls = ({ device, telemetry, commands }: CommandControlsProps) => {
  ...
  useEffect(() => {
    ...
    setDraft(first ? {
      state: 'draft',
      deviceId: device.id,
      intent: initialIntent(first),
      requiresConfirmation: first.requiresConfirmation,
    } : undefined);
  ...
  const requestSubmission = () => {
    if (draft.requiresConfirmation) setConfirmationOpen(true);
    ...
    <section className="device-card__commands" aria-label="Device commands">
      ...
      <div className="command-controls__form">
        ...
        <button type="button" onClick={requestSubmission} disabled={submitting}>
          {submitting ? 'Submitting…' : draft.requiresConfirmation ? 'Review command' : 'Send command'}
        </button>
      </div>
      {error && <p className="command-controls__error" role="alert">{error}</p>}
      {orderedCommands.length > 0 && (
        <div className="command-history" aria-label="Recent commands">
