...
### Проверка на границе этапов 10–11

Performance benchmark подтвердил архитектуру на 18 000/50 000 устройств. Измерениями обоснованы
один стабильный основной `IconLayer`, GPU status filtering и dirty-range updates. Spatial index,
clustering, worker, binary attributes и дополнительный culling не добавлены: Metal-прогон не
показал такого узкого места. Mobile profile остаётся эмуляцией на host GPU и записан как известное
ограничение, но не как незакрытая архитектурная задача MVP.

### Проверка на границе этапов 8–9
...
границами MVP (`A-010`, `A-019`), а не забытыми reliability requirements. Последующий performance
benchmark этапов 10–11 выполнен отдельно.
